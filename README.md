<h1 align="center">Hi 👋, I'm Simhadri Surada</h1>
<h3 align="center">A passionate competitive programmer from India 🇮🇳</h3>

> *"Learning through logic. Solving through code."*

---

### 📄 My Resume  
- [Click to view my resume](https://drive.google.com/file/d/1Efzzjqhzs4usakHqnxa7qvF8pIViJvGv/view?usp=sharing)

---
### 💼 Projects

- 🎮 **Tkinter Survival Game**  
  A 2-player game where players survive on a tile grid with random bombs and heals  
  [GitHub Repository](https://github.com/simhadrisurada/project)

- 🛡️ **Armor Safe Browsing Browser**  
  A Python-based browser that allows access only to verified websites

- 🧠 **Competitive Programming Practice**  
  Solving 100+ problems across platforms like LeetCode, CodeChef, and Codeforces
---
### Coding profiles 
- leetcode: [Simhadri_Surada](https://leetcode.com/u/simhadri_surada/)
-  Codechef: [Simhadri_Surada](https://www.codechef.com/users/parrot_king) 
---

### 🔭 Currently Working On:
- **Armor Safe Browsing Browser** – A Python-based secure browser  
- Personal projects and coding logs on GitHub

---

### 🌱 Currently Learning:
- **Tkinter**, Python GUI development  
- Basics of **AI/ML**  
- Improving problem-solving via DSA & competitive coding  

 ---

### 📫 Reach Me At
- 📧 Email: **narayanasurada1@gmail.com**  
- 💼 LinkedIn: [Simhadri Surada](https://www.linkedin.com/in/simhadri-surada-372ba8341/)

---

### 🛠️ Languages and Tools
- Python  
- C++  
- Linux  
- C   
- Ruby  
---



### 🧠 Fun Fact
> I enjoy solving logical problems, experimenting with creative Python projects, and exploring new tech like AI — one bug at a time!
